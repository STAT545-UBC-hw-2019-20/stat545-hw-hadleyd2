...
fig.cap = c('Figure 1: Boxplots of the country slope coefficients for GDP per capita versus year')
```
...
```{r max-gdp, fig.cap=fig.cap[1], fig.width=8, fig.height=4}
...
levels(gap.growth$continent)
gap.growth$continent <- with(gap.growth, fct_reorder(continent, -growth, median))
levels(gap.growth$continent)
```
...
gapminder %>% 
  group_by(country) %>% 
  summarize(year)
  # mutate(wavg.lifeExp = weighted.mean(lifeExp[gapminder$year <= year], pop[gapminder$year <= year])) %>% 
  # filter(year, country, continent, wavg.lifeExp)
  
# #working directory is project's directory
# file.name <- here::here("hw05", "hw05_files", "gap-hw5.csv")
# 
# #export csv to disc
# write_csv(gapminder, file.name)
# gapminder4hw <- read_csv(file.name)
```
