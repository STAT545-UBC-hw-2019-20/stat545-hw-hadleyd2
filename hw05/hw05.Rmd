...
fig.cap = c('Figure 1: Boxplots of the country slope coefficients for GDP per capita versus year.  The original factor order is alphabetical and the new factor order sorts descendingly by the median value',
            ...
            'Figure 3: Improving the Boxplot from Class Meeting 008 by changing the theme, editing the labels of both axes, adding a title, and reording the country factor',
            'Figure 4: Function ggsave saves the last created plot by default, the new and improved boxplot from Exercise 4',
            'Figure 5: By specifying the plot argument of ggsave(), we can save only the plot of interest, which is the original boxplot from Class Meeting 008')
```
...
For this exercise, we want to save the original plot from the class meeting 008 worksheet used in Exercise 4.  If we try the command `ggsave(improved.plot)` without specifying the `plot=` argument, the default behavior of `ggplot()` saves the new and improved plot only.
...
Note that the file was saved with the dimensions `fig.width=12, fig.height=5` that I had set in the previous code chunk.  The result of the default behavior shows the new and improved boxplot only in Figure 4.
